package com.uniconnect.student.modules.recommendation.serviceImpl;
...
import com.uniconnect.student.modules.recommendation.enums.MentorCategory;
import com.uniconnect.student.modules.recommendation.exception.ProfileIncompleteException;
import com.uniconnect.student.modules.recommendation.repository.MentorProfileRepository;
import com.uniconnect.student.modules.recommendation.repository.MentorRecommendationRepository;
import com.uniconnect.student.modules.recommendation.repository.StudentProfileRepository;
...
public class MentorRecommendationServiceImpl implements MentorRecommendationService {
    ...
    public List<RecommendationResponseDTO> generateRecommendations(Integer studentId) {
        ...
        // Industry recommendations are only for students with > 30 cumulative points.
        User user = userRepository.findById(studentId.longValue())
                .orElseThrow(() -> new ProfileIncompleteException(
                        "Student user not found. Please complete your account profile first."));
        int cumulativePoints = Optional.ofNullable(user.getCumulativePoints()).orElse(0);
        if (cumulativePoints <= 30) {
            recommendationRepository.deleteByStudentId(studentId);
            return Collections.emptyList();
        }

        // We require a department to match industry mentors.
        String studentDepartment = studentProfile.getDepartment();
        if (studentDepartment == null || studentDepartment.trim().isEmpty()) {
            return Collections.emptyList();
        }
        ...
        // Filter to only available industry mentors matching the student's department.
        List<MentorProfile> eligibleMentors = availableMentors.stream()
                .filter(m -> m.getMentorCategory() == MentorCategory.INDUSTRY_MENTOR)
                .filter(m -> m.getDepartment() != null && m.getDepartment().equalsIgnoreCase(studentDepartment))
        ...
        for (MentorProfile mentor : eligibleMentors) {
        ...
        return savedRecommendations.stream()
                ...
                .map(rec -> mapToResponseDTO(rec, studentProfile, eligibleMentors))
                .collect(Collectors.toList());
    }
    ...
    public List<RecommendationResponseDTO> getRecommendations(Integer studentId) {
        ...
        // Industry recommendations only for students with > 30 cumulative points.
        User user = userRepository.findById(studentId.longValue())
                .orElseThrow(() -> new ProfileIncompleteException(
                        "Student user not found. Please complete your account profile first."));
        int cumulativePoints = Optional.ofNullable(user.getCumulativePoints()).orElse(0);
        ...
        List<MentorProfile> allowedIndustryMentors = mentorProfileRepository.findByIsAvailableTrue().stream()
                .filter(m -> m.getMentorCategory() == MentorCategory.INDUSTRY_MENTOR)
                .filter(m -> m.getDepartment() != null && m.getDepartment().equalsIgnoreCase(studentDepartment))
                .collect(Collectors.toList());

        Set<Integer> allowedMentorIds = allowedIndustryMentors.stream()
                .map(MentorProfile::getMentorId)
                .collect(Collectors.toSet());

        List<MentorRecommendation> recommendations =
                recommendationRepository.findByStudentIdOrderByMatchScoreDesc(studentId)
                        .stream()
                        .filter(rec -> allowedMentorIds.contains(rec.getMentorId()))
                        .collect(Collectors.toList());

        // Load all mentor profiles for mapping
        List<MentorProfile> allMentors = allowedIndustryMentors;
