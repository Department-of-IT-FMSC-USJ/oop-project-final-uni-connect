package com.uniconnect.student.modules.feedback.controller;
...
public class MentorFeedbackController {
    ...
    /**
     * Get completed mentor sessions for a student that still need feedback.
     *
     * GET /api/feedback/pending/{studentId}
     */
    @GetMapping("/pending/{studentId}")
    public ResponseEntity<ApiResponseDTO<List<PendingFeedbackSessionDTO>>> getPendingCompletedSessions(
            @PathVariable("studentId") Integer studentId) {

        List<PendingFeedbackSessionDTO> pendingSessions =
                mentorFeedbackService.getPendingCompletedSessions(studentId);

        ApiResponseDTO<List<PendingFeedbackSessionDTO>> response = ApiResponseDTO.success(
                "Pending sessions retrieved successfully", pendingSessions);
